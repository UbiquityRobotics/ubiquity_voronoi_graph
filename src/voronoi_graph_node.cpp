...
#include <voronoi_graph/voronoi_graph_node.h>
#include <voronoi_graph/voronoi_graph_generator.h>
#include <memory>
...
namespace tuw_graph
{
    VoronoiGeneratorNode::VoronoiGeneratorNode(ros::NodeHandle &n) : voronoi_map::VoronoiPathGenerator(), VoronoiGraphGenerator(), Serializer(), n_(n), n_param_("~")
    {
        ...
        double loop_rate;
        //try to retrieve parameter from parameter server. If the parameter cannot be retrieved, default is returned instead 
        ...
        n_param_.param<float>("segment_length", segment_length_, 2.5); /// [meters]
        ...
        pubSegments_ = n.advertise<ubiquity_voronoi_graph::Graph>("segments", 1);
    ...
    void VoronoiGeneratorNode::globalMapCallback(const nav_msgs::OccupancyGrid::ConstPtr &_map)
    {
        ...
        // generate hash number as folder name for voronoi graph
    ...
    void VoronoiGeneratorNode::createGraph(const nav_msgs::OccupancyGrid::ConstPtr &_map, size_t _map_hash)
    {
        ...
        // ROS_INFO("Saving map");
        // cv::imwrite("/home/jajelinx/Desktop/voronoi/map.jpg", m);
        // std::ofstream map_file;
        // map_file.open("/home/jajelinx/Desktop/voronoi/map.txt", std::ios_base::app);
        // map_file << m; 
        // map_file.close();

        prepareMap(m, map_, inflation_ / _map->info.resolution);
        
        // ROS_INFO("Saving smoothed map");
        // cv::imwrite("/home/jajelinx/Desktop/voronoi/smooth_map.jpg", map_);
        // std::ofstream smooth_map_file;
        // smooth_map_file.open("/home/jajelinx/Desktop/voronoi/smooth_map.txt", std::ios_base::app);
        // smooth_map_file << map_;
        // smooth_map_file.close();

        computeDistanceField(map_, distField_);
        cv::imwrite("/home/jajelinx/Desktop/voronoi/dist_field.jpg", distField_);

        ROS_INFO("Graph generator: Computing voronoi graph ...");
        computeVoronoiMap(distField_, voronoiMap_);

        ROS_INFO("saving voronoi graph data ...");
        std::ofstream voronoi_map_file;
        voronoi_map_file.open("/home/jajelinx/Desktop/voronoi/voronoi_map.txt", std::ios_base::app);
        voronoi_map_file << voronoiMap_;
        voronoi_map_file.close();
    ...
    {
        ubiquity_voronoi_graph::Graph graph;
        ...
        {
            ubiquity_voronoi_graph::Vertex seg;
